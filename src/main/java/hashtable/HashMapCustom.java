...
public class HashMapCustom<K, V> {
    ...
    public void put(K newKey, V data) {
        ...
        Object [] table = matrrix[row(newKey)];
        //if table location does not contain any entry, store entry there.
        if (table[hash] == null) {
            table[hash] = newEntry;
        } else {
            Entry<K, V> previous = null;
            Entry<K, V> current = (Entry<K, V>) table[hash];
    ...
    public V get(K key) {
        int hash = hash(key);
        Object [] table = matrrix[row(key)];
        if (table[hash] == null) {
            return null;
        } else {
            Entry<K, V> temp = (Entry<K, V>) table[hash];
            while (temp != null) {
    ...
    public boolean remove(K deleteKey) {
        ...
        Object [] table = matrrix[row(deleteKey)];
        if (table[hash] == null) {
            return false;
        } else {
            Entry<K, V> previous = null;
            ...
            while (current != null) { //we have reached last entry node of bucket.
                if (current.key.equals(deleteKey)) {
                    if (previous == null) {  //delete first entry node.
                        table[hash] = ((Entry<K,V>)table[hash]).next;
                        return true;
                    } else {
